...
export default function CloseOutFloorPlan({
  ...
  const canShowFloorPlan =
    state.surveyType === "closeout" && !!(state.school?.hasFloorPlan || state.floorPlan)

  const floorPlanOpen = canShowFloorPlan && open
  ...
  const handleRequestSelectRoom = useCallback(
    (roomId: string) => {
      requestSelectRoom(roomId, {
        afterSelect: onClose,
        onChooseDifferent: onOpen,
      })
    },
    [requestSelectRoom, onClose, onOpen],
  )

...
export function validateRoomSession(
  ...
  if (roomSession.sourceSurveyType && (roomSession.pendingQuestionIds || roomSession.pendingGrade)) {
    ...
    const missingGrade =
      studioTypeRequiresGrade(roomSession.roomType) &&
  ...
  // Grade is only required for Traditional studio; deferred rooms can finish it in Close Out.
  const missingGrade =
    studioTypeRequiresGrade(roomSession.roomType) &&
    !roomSession.gradeType &&
...
export function validateSurveyBeforeDeferral(
  ...
  for (const roomId of roomIds) {
    ...
    const result = validateRoomSession(roomId, parsed?.name ?? roomId, forCheck, rubric.questions)
    if (!result.complete) incompleteRooms.push(result)
  }

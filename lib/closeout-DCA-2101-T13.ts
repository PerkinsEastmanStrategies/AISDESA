...
export function deferIncompleteToCloseOut(
  ...
  for (const [roomId, roomSession] of roomEntries) {
    ...
    const result = validateRoomSession(roomId, parsed?.name ?? roomId, roomSession, rubric.questions)
    ...
    closeRooms[roomId] = {
      ...
      pendingGrade:
        pendingGrade ||
        (!!existingClose?.pendingGrade && studioTypeRequiresGrade(roomSession.roomType)),
    }

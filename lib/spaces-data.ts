 ...
 * - CAFM label-only exports: `#CAFM_ID` without `#CAFM_SPACE` (synthetic hit targets)
...
export function extractRoomsFromSvg(svgContent: string | null): RoomInfo[] {
  ...
  try {
    ...
    try {
      ...
      const legacyRooms = extractLegacyRooms(clone);
      if (legacyRooms.length > 0) return legacyRooms;

      return extractProomRooms(clone);
    } finally {
...
/**
 * Square hit target around a label centroid (used when #CAFM_SPACE is missing).
 * Size is based on nearest-neighbor spacing so targets stay tappable but rarely overlap.
 */
function syntheticLabelHitPolygon(
  x: number,
  y: number,
  halfSize: number
): { x: number; y: number }[] {
  const h = Math.max(halfSize, 1);
  return [
    { x: x - h, y: y - h },
    { x: x + h, y: y - h },
    { x: x + h, y: y + h },
...
function estimateLabelHotspotHalfSize(
  labels: CafmLabel[],
  svgRoot: SVGSVGElement
): number {
  const planScale = getPlanDistanceScale(svgRoot);
  // ~4–7% of plan extent → roughly 24–40px on a ~400px-wide panel for Allison-scale SVGs.
  const minHalf = planScale * 0.04;
  const maxHalf = planScale * 0.07;
  const fallback = Math.max(minHalf, 48);
  ...
  for (let i = 0; i < labels.length; i++) {
    ...
    for (let j = 0; j < labels.length; j++) {
      if (i === j) continue;
      const d = Math.sqrt(
        distanceSquared(labels[i].x, labels[i].y, labels[j].x, labels[j].y)
      );
      if (d > 1 && d < best) best = d;
    }
    if (Number.isFinite(best) && best < Infinity) nearestGaps.push(best);
  }

  if (nearestGaps.length === 0) return fallback;

  nearestGaps.sort((a, b) => a - b);
  const median = nearestGaps[Math.floor(nearestGaps.length / 2)];
  // Prefer a tappable target; only shrink when labels are very densely packed.
  return Math.min(Math.max(median * 0.55, minHalf), maxHalf);
}

/**
 * Plans like Allison ES ship `#CAFM_ID` room numbers without a `#CAFM_SPACE`
 * boundary layer (walls are open strokes). Build selectable rooms at label positions.
 */
function extractCafmLabelHotspotRooms(svgRoot: SVGSVGElement): RoomInfo[] {
  const labels = getCafmLabels(svgRoot);
  if (labels.length === 0) return [];

  const halfSize = estimateLabelHotspotHalfSize(labels, svgRoot);
  const roomsByKey = new Map<string, RoomInfo & { priority: number }>();

  for (const label of labels) {
    const key = label.kind === "room" ? label.key : label.text;
    // Prefer exact room numbers (101) over stripped suffixes (101RR → 101) for hotspot placement.
    const priority =
      label.priority + (/^\d{3}[A-Z]?$/.test(label.text) ? 10 : 0);
  ...
  const rooms: RoomInfo[] = Array.from(roomsByKey.values()).map(
    ({ priority: _priority, ...room }) => room
  );
...
function extractCafmRooms(svgRoot: SVGSVGElement): RoomInfo[] {
  ...
  // Label-only CAFM exports (e.g. Allison ES): no space polygons to join.
  if (!cafmSpace) {
...
/** Suffixes that mark a separate service space — do not collapse onto the parent room id. */
const CAFM_SERVICE_SUFFIXES = new Set([
  "STO",
  "RR",
  "HRR",
  "BRR",
...
/**
 * Room IDs from CAFM labels. Prefer keeping instructional IDs intact.
 * Service adjunct labels (B111HRR) stay as their own keys so they can be filtered out.
 */
function cafmRoomKeyFromLabel(rawText: string): { key: string; kind: "room" | "tag" } {
  ...
  // Studio-style IDs (S1-G1)
  if (/^S\d+-[A-Z0-9]+$/.test(text)) {
    return { key: text, kind: "room" };
  }

  // Wing-style elementary IDs (Cowan: E1, N12, W8)
  if (/^[ENW]\d{1,2}$/.test(text)) {
    return { key: text, kind: "room" };
  }
  ...
  if (numericSuffixed) {
    ...
    return { key: numericSuffixed[1], kind: "room" };
  }
  ...
  if (buildingSuffixed) {
    ...
    return { key: buildingSuffixed[1], kind: "room" };
  }

  // Anything else that looks like an ID — keep as room candidate; picker filter decides.
  ...
  if (audit.ok) {
    ...
    console.info(
      `${prefix} OK — ${audit.parsedLabelCount} labels, ${audit.labeledRegionCount} labeled regions, ${audit.totalShapeCount} shapes` +
        (audit.hasCafmId && !audit.hasCafmSpace
          ? " (CAFM_ID only — room hotspots from labels)"
          : "") +
...
export function findRoomMatchAtSvgCoordinates(
  ...
  if (cafmSpace || cafmId) {
    const region = cafmSpace
      ? findRegionAtSvgCoordinates(svgRoot, x, y)
      : null;
    const labels = getCafmLabels(svgRoot);
    const room =
      labels.length > 0
        ? (cafmSpace ? findRoomInCafmRegions(svgRoot, x, y) : null) ??
          findRoomAtPoint(extractCafmLabelHotspotRooms(svgRoot), x, y) ??
...
export function findRoomAtSvgCoordinates(
  ...
  if (cafmSpace || cafmId) {
    ...
    if (labels.length > 0) {
      ...
      const fromHotspot = findRoomAtPoint(
        extractCafmLabelHotspotRooms(svgRoot),
        x,
        y
      );
...
/** Lively / ESA plans with `<polygon class="proom" data-i="…">`. */
function extractProomRooms(svgRoot: SVGSVGElement): RoomInfo[] {
  const nodes = Array.from(
    svgRoot.querySelectorAll("polygon.proom, #planRooms polygon.proom")
  ...
  for (const poly of nodes) {
    const key = poly.getAttribute("data-i")?.trim() ?? poly.getAttribute("data-label")?.trim();
    ...
    rooms.push({
      ...
      points,
      label: poly.getAttribute("data-label") ?? undefined,
    });
  }
